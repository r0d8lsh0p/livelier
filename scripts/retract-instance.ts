 ...
 *
 * Usage: npx ts-node scripts/retract-instance.ts <url> [--confirm]
...
import { LiveEventPublisher } from '../src/core/nostr/live-event.publisher';
import { bridgeSignerFrom, instanceSigner } from '../src/core/identity';
import { profileWriteRelays } from '../src/core/relays';
...
async function main(): Promise<void> {
  ...
  if (!url) {
    console.error('Usage: npx ts-node scripts/retract-instance.ts <url> [--confirm]');
    process.exit(2);
  }
